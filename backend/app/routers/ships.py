...
def add_ship_to_user(data: schemas.ShipBase, authorization: str = Header(), db: Session = Depends(get_db)):
...
def get_ships_by_user_id(authorization: str = Header(), db: Session = Depends(get_db)):
    decoded_jwt = jwt.decode(authorization, SECRET_KEY, algorithms=[HASHING_ALGORITHM])
    user_id = decoded_jwt.get('user_id')
